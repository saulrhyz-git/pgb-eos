  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-base font-semibold text-slate-800 dark:text-slate-100">Custom Roles</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            ...
            Collections, Expenses, Rocks, Disbursements, the Executive Scorecard, the Audit Log, Reports, and the
            Comparison tab there. Disbursements works just like Rocks — a single combined grant covering all three
            sub-categories (Advances/Loans/Interests) together, rather than three separate resources the way
            Revenue/Collections/Expenses are. Only the View column matters for the Executive Scorecard, the Audit
            Log, Reports, and Comparison — all four are read-only pages, so Edit/Delete checkboxes for them are
            ignored. The Audit Log, Reports, and Comparison aren't really scoped to any one Business Unit/Company
            the way the others are (the Audit Log is a single global log; Reports and Comparison are each their own
            page-level gate, with the underlying figures still masked by whatever Revenue/Collections/Expenses/
            Rocks/Disbursements grants apply), so granting View on any one row here is enough to open that page at
            all — they're just attached to a scope here to reuse the same picker.
          </p>
        </div>
        <button
          onClick={startCreate}
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-sm sm:p-6">
          <div className="flex items-center justify-between">
            <div className="text-sm font-semibold text-slate-700 dark:text-slate-200">{form.id ? "Edit Role" : "New Role"}</div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-slate-500 dark:text-slate-400">Role Name</label>
              <input
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[260px_1fr]">
            ...
            <div className="flex flex-col gap-1 rounded-md border border-slate-200 dark:border-slate-700 p-3">
              <div className="mb-1 text-xs font-medium text-slate-500 dark:text-slate-400">Business Units &amp; Companies</div>
                  ...
                  <div key={bu.id}>
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="rounded p-0.5 text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700"
                      >
                        {expandedBus.has(bu.id) ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
                      </button>
                      <label className="flex flex-1 cursor-pointer items-center gap-2 rounded px-1.5 py-1 text-sm hover:bg-slate-50 dark:hover:bg-slate-800">
                        <input
                          type="checkbox"
                          checked={isScopeSelected(bu.id)}
                          onChange={() => toggleScope(bu)}
                        />
                        <span className="font-medium text-slate-700 dark:text-slate-200">{bu.name}</span>
                      </label>
                    </div>
                    {expandedBus.has(bu.id) && (
                      <div className="ml-7 flex flex-col gap-0.5 border-l border-slate-100 dark:border-slate-800 pl-2">
                        {(bu.companies || []).map((c) => (
                          <label
                            key={c.id}
                            className="flex cursor-pointer items-center gap-2 rounded px-1.5 py-1 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800"
                          >
                            <input
                              type="checkbox"
                        ...
                        {(bu.companies || []).length === 0 && (
                          <span className="px-1.5 py-1 text-xs text-slate-400 dark:text-slate-500">No companies yet</span>
                        )}
                      </div>
                    )}
                  </div>
                ))}
                {businessUnits.length === 0 && <span className="px-1.5 py-1 text-xs text-slate-400 dark:text-slate-500">No Business Units yet</span>}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              {scopeEntries.length === 0 && (
                <div className="flex h-full items-center justify-center rounded-md border border-dashed border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 p-6 text-sm text-slate-400 dark:text-slate-500">
                  Select a Business Unit or Company on the left to grant it View/Edit/Delete access.
                </div>
              )}
              {scopeEntries.map((entry) => (
                <div key={entry.key} className="rounded-md border border-slate-200 dark:border-slate-700 p-3">
                  <div className="mb-2 flex items-center justify-between">
                    <div className="text-sm font-medium text-slate-700 dark:text-slate-200">
                      {entry.companyName ? (
                        <>
                          {entry.businessUnitName} <span className="text-slate-400 dark:text-slate-500">/</span> {entry.companyName}
                        </>
                      ) : (
                        <>
                          {entry.businessUnitName} <span className="text-xs font-normal text-slate-400 dark:text-slate-500">(whole Business Unit)</span>
                        </>
                      )}
                    </div>
                    <button
                      ...
                      className="rounded p-1 text-slate-400 dark:text-slate-500 hover:bg-red-50 dark:hover:bg-red-900/40 hover:text-red-600 dark:hover:text-red-400"
                      ...
                      <tbody>
                        {RESOURCES.map((resource) => (
                          <tr key={resource} className="border-t border-slate-100 dark:border-slate-800">
                            <td className="py-1.5 text-slate-600 dark:text-slate-300">{RESOURCE_LABELS[resource]}</td>
            ...
            {roles.map((role) => {
              ...
              return (
                <tr key={role.id}>
                  ...
                  <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{role.userCount}</td>
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-2">
                      <button onClick={() => startEdit(role)} className="rounded-md p-1.5 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700" title="Edit">
