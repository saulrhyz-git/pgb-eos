...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (narrowingApplies(Boolean(req.user!.role), permRows, ["ROCKS"])) assertPermission(permRows, "edit", "ROCKS", { businessUnitId, companyId: parsed.data.companyId });
...
router.post("/rollover", requireRole("GROUP_INTEGRATOR", "SUPERADMIN"), async (req, res) => {
  ...
  try {
    if (companyId) {
      const buId = await resolveCompanyBusinessUnit(companyId);
      assertBusinessUnitAccess(user, buId);
      if (narrowingApplies(Boolean(user.role), permRows, ["ROCKS"])) assertPermission(permRows, "edit", "ROCKS", { businessUnitId: buId, companyId });
    }
  } catch (err: any) {
    return res.status(err.status || 500).json({ error: err.message });
  }
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    assertBusinessUnitAccess(req.user!, businessUnitId);
    const permRows = await loadUserPermissions(req.user!);
    if (narrowingApplies(Boolean(req.user!.role), permRows, ["ROCKS"])) assertPermission(permRows, "edit", "ROCKS", { businessUnitId, companyId: existing.companyId });
    if (parsed.data.businessGoalId) await assertBusinessGoalUsable(parsed.data.businessGoalId, businessUnitId);
  } catch (err: any) {
    return res.status(err.status || 500).json({ error: err.message });
  }
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    assertBusinessUnitAccess(req.user!, businessUnitId);
    const permRows = await loadUserPermissions(req.user!);
    if (narrowingApplies(Boolean(req.user!.role), permRows, ["ROCKS"])) assertPermission(permRows, "delete", "ROCKS", { businessUnitId, companyId: existing.companyId });
  } catch (err: any) {
    return res.status(err.status || 500).json({ error: err.message });
  }
